...
object CurriesPartialAppliedFunctions extends App {
  ...
  // lifting = ETA-EXPANSION = convert method to function

  // functions != methods (JVM limitation)
  def inc(x: Int) = x + 1
  List(1,2,3).map(x => inc(x))  // ETA-expansion - convert inc to x => inc(x)

  // Partial applied function = PAF
  val add5 = curriedAdder(5) _ // Int => Int, if _ compiler transform this to valid function

  /* Three ways to add 2 numbers */
  val simpleAddFunction = (x: Int, y: Int) => x + y
  def simpleAddMethod(x: Int, y: Int) = x + y
  def curriedAddMethod(x: Int)(y: Int) = x + y

  // Implement in different ways add7: Int => Int = 7 + y
  ...
  val add7_5 = simpleAddMethod(7, _: Int) // alternative syntax for turning methods into function values ETA-expansion
  // y => simpleAddMethod(7, y)

  // underscores are powerful
  def concatenator(a: String, b: String, c: String) = a + b + c
  val insertName = concatenator("Hello, I'm ", _: String, ", how are you?") // transformed to x: String => concatenator(hello, x, howarewyou)
  println(insertName("Daniel"))

  val fillInTheBlanks = concatenator("Hello, ", _: String, _: String) // transformed to  (x, y) => concatenator("Hello, ", x, y)
